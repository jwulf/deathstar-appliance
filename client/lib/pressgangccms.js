    ...
    var PressGangClass = (function () {
        ...
        PressGangCCMS.prototype.isContentSpec = function (topic_id, cb) {
            ...
            if(typeof topic_id !== 'number') {
                errmsg = 'Need numeric Topic ID as first argument';
                cb(errmsg, false);
                return {error: errmsg};
            }
        ...
        PressGangCCMS.prototype.getTopicData = function (data_request, topic_id, revORcb, cb) {
            ...
            if(!exports.DATA_REQ[data_request]) {
                errmsg = 'Unsupported operation ' + data_request + ' passed as first argument'
                cb(errmsg, null);
                return {error: errmsg};
            }
            if('number' !== typeof parseInt(topic_id)) {
                errmsg = 'Need numerical Topic ID as second argument';
                cb(errmsg, null);
                return {error: errmsg};
            }
            _topic_id = parseInt(topic_id);
            ...
            this.getBaseRESTPath(function getBaseRESTPathCallback (requestPath) {
                switch(data_request) {
                    ...
                    case exports.DATA_REQ.json: {
                        requestPath += '/topic/get/json/' + _topic_id;
                        if(_rev) {
                            requestPath += '/r/' + _rev;
                        }
                        break;
